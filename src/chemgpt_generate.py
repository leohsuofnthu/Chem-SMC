"""
ChemGPT-4.7M generation with optimized prompts for molecular design.
"""
...
def generate_chemgpt(
    ...
    # Create progress bar for ChemGPT generation
    pbar = tqdm(
        total=n,
        desc="Generating ChemGPT molecules",
        unit="mol",
        ncols=80,
...
def run_experiment(
    ...
    **gen_kwargs,
    ...
    exp_pbar = tqdm(
        ...
        desc="Running ChemGPT experiments",
        unit="exp",
        ncols=80,
        bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
    )
...
def main() -> None:
    ...
    parser.add_argument("--out_csv", type=str, default="results/chemgpt_results.csv")
    parser.add_argument("--summary_csv", type=str, default="results/chemgpt_summary.csv")
